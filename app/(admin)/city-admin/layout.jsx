...
export default function CityAdminLayout({ children }) {
      ...
      >
        ...
        >
          ...
          <p className="text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">
            ...
            , you don’t have permission to access the City Admin Dashboard. Please
            continue to your{" "}
            <span className="text-indigo-600 font-semibold capitalize">
              {dbRole ? dbRole.replace("_", " ") : "assigned"}
            </span>{" "}
            dashboard.
          </p>
          ...
          {/* Button */}
          <button
            onClick={() => router.push(dbRole ? `/${dbRole.replace("_", "-")}` : "/staff/sign-in")}
            className="flex items-center justify-center gap-2 w-full
            bg-gradient-to-r from-indigo-600 to-violet-600
            hover:from-indigo-700 hover:to-violet-700
            text-white px-6 py-3 rounded-xl
            transition-all duration-300 shadow-lg hover:shadow-indigo-500/25 cursor-pointer"
          >
            Go to Dashboard
