...
function calculateStreak(previousLastActivityAt) {
  const now = Date.now();
...
function isStreakEligibleAction(type) {
  return ["issue_submitted", "video_evidence_added", "comment_added"].includes(
    type,
  );
}

function getCitizenCriteriaValue(citizen, criteriaType) {
  switch (criteriaType) {
...
export async function awardCitizenPoints(ctx, args) {
...
export async function awardBadgeIfNotExists(ctx, args) {
...
export async function checkAndAwardCitizenBadges(ctx, args) {
  const citizen = await ctx.db.get(args.citizenId);
