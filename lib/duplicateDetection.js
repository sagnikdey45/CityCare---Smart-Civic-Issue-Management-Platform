...
function overlapSimilarity(wordsA, wordsB) {
  if (!wordsA.length || !wordsB.length) return 0;
...
function getProximitySimilarity(distanceMeters) {
...
function getDuplicateLevel(score) {
...
function getSubcategoryResult(subA = [], subB = []) {
...
function buildMobileIssue(issue) {
  ...
  return {
    ...
    dateReported:
      issue.createdAt ?? issue.dateReported ?? new Date().toISOString(),
    createdAt:
      issue.createdAt ?? issue.dateReported ?? new Date().toISOString(),
...
function createGroupReason(pairMetrics) {
  const bestPair = [...pairMetrics].sort(
    (a, b) => b.duplicateScore - a.duplicateScore,
  )[0];
...
function getConnectedComponents(issueIds, matchingPairs) {
...
export function buildDuplicateGroupsFromIssues(rawIssues) {
  const issues = rawIssues
    .map(buildMobileIssue)
    .filter((issue) => Boolean(issue?.reportedBy));
  ...
  issuesByCitizen.forEach((citizenIssues, citizenId) => {
    ...
    components.forEach((componentIds, index) => {
      const componentIssues = citizenIssues.filter((issue) =>
        componentIds.includes(issue.id),
      );

      const componentPairs = pairMetrics.filter(
        (pair) =>
          componentIds.includes(pair.issueAId) &&
          componentIds.includes(pair.issueBId),
      );

      if (componentIssues.length < 2 || componentPairs.length < 1) return;

      const bestOverallScore = Math.max(
        ...componentPairs.map((p) => p.overallScore),
      );
      const bestDuplicateScore = Math.max(
        ...componentPairs.map((p) => p.duplicateScore),
      );
      const averageOverallScore =
        componentPairs.reduce((sum, pair) => sum + pair.overallScore, 0) /
        componentPairs.length;
      const minimumDistanceMeters = Math.min(
        ...componentPairs.map((p) => p.distanceMeters),
      );
      ...
      groups.push({
        ...
        citizenId,
        citizenName: firstIssue.citizenName,
        citizenEmail: firstIssue.citizenEmail,
  ...
  return groups.sort(
    (a, b) =>
      b.similarityMetrics.bestDuplicateScore -
      a.similarityMetrics.bestDuplicateScore,
  );
}

export function getDuplicateFlagsByIssueId(rawIssues, issueId) {
  if (!issueId) {
    return {
      issueId,
  ...
  const matchedGroups = duplicateGroups
    ...
    .map((group) => {
      ...
      const relatedIssues = group.issues.filter(
