...
import { Shield, Download, RefreshCw, Activity, Loader2 } from "lucide-react";
...
export default function CityAdminAuditLogs({ cityAdminUserId }) {
  ...
  // Query city-scoped audit logs
  const auditData = useQuery(
    api.auditLogs.getCityAdminAuditLogs,
    cityAdminUserId
      ? {
          cityAdminUserId,
  ...
  const handleExportCSV = () => {
    ...
    const filename = `CityCare_Audit_${scope.city || "City"}_${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-4">
          <div className="p-3.5 rounded-2xl bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow-md shadow-blue-500/20">
            <Shield className="w-7 h-7" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
              City Audit Logs & Accountability
            </h1>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Complete operational audit trail for {scope.city || "your city"}{" "}
      ...
      {isLoading ? (
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-16 text-center border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-xs font-semibold text-slate-600 dark:text-slate-400">
            Loading city audit records...
          </p>
        </div>
      ) : (
        <>
          {/* Summary Cards */}
          <AuditSummaryCards summary={summary} isSystemAdmin={false} />
